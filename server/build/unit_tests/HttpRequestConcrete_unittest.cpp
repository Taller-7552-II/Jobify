...
TEST(HttpRequestConcreteTests, GetURIParsed){
    ...
    struct http_message* hmsg = new_http_message("GET", "users/gonzalo", "");
    req.init(conn, hmsg);
    req.getUriParsed(&parsed);
    EXPECT_TRUE(parsed[0] == "users");
    EXPECT_TRUE(parsed[1] == "gonzalo");
    delete_http_message(hmsg);
...
TEST(HttpRequestConcreteTests, GetURIStringParsedByIndex){
    ...
    EXPECT_TRUE(req.getUriStringParsedByIndex(1) == "gonzalo");
    delete_http_message(hmsg);
...
TEST(HttpRequestConcreteTests, GetCampo){
    ...
    struct http_message* hmsg = new_http_message("GET", "users/gonzalo", "{\"user\":\"gonzalo\"}");
    req.init(conn, hmsg);
    EXPECT_TRUE(req.getCampo("user") == "gonzalo");
    delete_http_message(hmsg);
}
...
TEST(HttpRequestConcreteTests, GetCampoDeArray){
    ...
    struct http_message* hmsg = new_http_message("GET", "users/gonzalo", "{\"users\":[\"gonzalo\", \"juampa\"]}");
    req.init(conn, hmsg);
    EXPECT_TRUE(req.getCampoDeArray("users", 0) == "gonzalo");
    EXPECT_TRUE(req.getCampoDeArray("users", 1) == "juampa");
    delete_http_message(hmsg);
}
...
TEST(HttpRequestConcreteTests, GetUsername){
    ...
    struct http_message* hmsg = new_http_message("GET", "users/gonzalo", "");
    req.init(conn, hmsg);
    EXPECT_TRUE(req.getUsername() == "gonzalo");
    delete_http_message(hmsg);

    hmsg = new_http_message("GET", "users/matias", "");
    req.init(conn, hmsg);
...
TEST(HttpRequestConcreteTests, GetFilename){
    ...
    struct http_message* hmsg = new_http_message("GET", "files/gonzalo/test.txt", "");
    req.init(conn, hmsg);
...
TEST(HttpRequestConcreteTests, GetResponseOK){
    struct mg_connection* conn = new struct mg_connection;
    HttpRequestConcrete req;
    Reader reader;
...
TEST(HttpRequestConcreteTests, AddValueToBody){
    ...
    req.setResponse(Status::Aborted());
    req.addValueToBody("conn_token", "qwerty");
    response = req.getResponse();
    reader.parse(response.c_str(), response_parsed);
    EXPECT_TRUE(response_parsed["conn_token"] == "qwerty");
    EXPECT_TRUE(req.getStatusCode() != 200);
...
TEST(HttpRequestConcreteTests, GetHeaderValue){
    ...
    header_values.push_back("qwerty");
    struct http_message* hmsg = new_http_message("GET", "files/gonzalo/test.txt", "", &header_names, &header_values);
    req.init(conn, hmsg);
