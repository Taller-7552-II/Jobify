...
TEST(JobGetHandlerTests, GetJobCategoriaOK){
	...
	struct http_message* hmsg = new_http_message("GET", "/job/software", "", &header_names, &header_values);
...
TEST(JobGetHandlerTests, GetJob1CampoOK){
	Database* db = new DatabaseMockRAM;
	TokenAuthenticator* tk = new TokenAuthenticator;
	string token = tk->createToken("gonzalo");
	JobGetHandler handler(db, tk);
	...
	header_values.push_back(token);
	struct mg_connection* conn = new struct mg_connection;
	struct http_message* hmsg = new_http_message("GET", "/job/", "", &header_names, &header_values);
