...
void UserModifyHandler::_handle(HttpRequest &hmsg){
    ...
    std::string jobz = hmsg.getCampo("jobList");
    log->Log("El campo recibido por jobList es : "+jobz,DEBUG);
    std::string skillz = hmsg.getCampo("skillList");
    log->Log("El campo recibido por skillList es : "+skillz,DEBUG);
    ...
    if (s.ok()) s = user_metadata.DBchange_job_list(jobz);
    if (s.ok()) s = user_metadata.DBchange_skill_list(skillz);
    // ver status
    if(s.ok()){
