...
SkillGetHandler::SkillGetHandler(Database *db, TokenAuthenticator *a) : EventHandlerIgnoresAuthentication(db, a)
{
    //ctor
}

SkillGetHandler::~SkillGetHandler()
{
    //dtor
}

bool SkillGetHandler::isMyRequest(HttpRequest &hmsg){
    // GET /job/'categoria' quiere decir pedir info del usuario
    Server_Logger* log = Server_Logger::getInstance();
    log->Log("Verifica que se trate de un Handler tipo SkillGetHandler",INFO);
    if(hmsg.getUriParsedByIndex(0) == HttpRequest::SKILL &&
        hmsg.getUriCantCampos() <= 2 &&
        hmsg.getUriType() ==  HttpRequest::ELEMENT_URI &&
        hmsg.getMethod() == HttpRequest::GET){
        log->Log("Confirma que es un Handler tipo SkillGetHandler",INFO);
        return true;
    }
    return false;
}

void SkillGetHandler::_handle(HttpRequest &hmsg){
    Status s = Status::OK();
